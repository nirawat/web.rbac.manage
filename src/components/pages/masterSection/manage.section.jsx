...
const ManageMasterSection = (props) => {
  const { t } = useTranslation();
  const dispatch = useDispatch();
  const classes = useStyles();
  const { isOpen, isMode, isData, handleCloseDialog } = props;
  const initialData = useSelector((state) => state.reducerSection.model);
          ...
          <form className={classes.root} autoComplete="off">
            <Grid container spacing={3}>
              <Grid item xs={12} sm={12}>
                <TextField
                  label={t("master_section_code")}
                  id="master_section_code"
                  required={true}
              ...
              <Grid item xs={12} sm={12}>
                <TextField
                  label={t("master_section_name_thai")}
                  id="master_section_name_thai"
                  required={true}
              ...
              <Grid item xs={12} sm={12}>
                <TextField
                  label={t("master_section_name_eng")}
                  id="master_section_name_eng"
                  required={true}
...
export default ManageMasterSection;
