...
const ManageMasterGroup = (props) => {
  const { t } = useTranslation();
  const dispatch = useDispatch();
  const classes = useStyles();
  const { isOpen, isMode, isData, handleCloseDialog } = props;
  const initialData = useSelector((state) => state.reducerMasterGroup.model);
  const CategoryData = useSelector(
    (state) => state.reducerMasterCategory.data
  );
  ...
  const handleSelectCategoryOpen = () => {
  ...
  function asyncCategory() {
    (async () => {
      await globalHandleService.GlobalRequest(dispatch, TYPE_MASTER_CATEGORY);
    })();
  }
          ...
          <form className={classes.root} autoComplete="off">
            <Grid container spacing={3}>
              <Grid item xs={12} sm={12}>
                <TextField
                  label={t("master_group_code")}
                  id="master_group_code"
                  required={true}
              ...
              <Grid item xs={12} sm={12}>
                <TextField
                  label={t("master_group_name_thai")}
                  id="master_group_name_thai"
                  required={true}
              ...
              <Grid item xs={12} sm={12}>
                <TextField
                  label={t("master_group_name_eng")}
                  id="master_group_name_eng"
                  required={true}
              ...
              <Grid item xs={12} sm={12}>
                <FormControl className={classes.formControl}>
                  ...
                  <Select
                    ...
                    onOpen={handleSelectCategoryOpen}
                    value={masterData.category_code}
                    disabled={isMode === REMOVE ? true : false}
                    onChange={(e) => {
                      setMasterData({
                        ...masterData,
                        category_code: e.target.value,
                        category_name: e._targetInst.memoizedProps.name,
                      });
                    }}
                  >
...
export default ManageMasterGroup;
