//GLOBAL SELECT ROW TABLE
export const THEME_LIGHT = "light";
export const THEME_DARK = "dark";
export const THEME_COLORS = "colors";
export const THEME_CONTRAST_TEXT = "#f5f5f5";
export const THEME_COLOR_DEFAULT = "#455a64";

export const THEME_COLOR_BLUE = "#2196f3";
export const THEME_COLOR_BLUE_GRAY = "#455a64";
export const THEME_COLOR_GREEN = "#469a10";
export const THEME_COLOR_AMBER = "#ffc107";
export const THEME_COLOR_ORANG = "#ff9800";
export const THEME_COLOR_RED = "#f44336";
export const THEME_COLOR_PURPLE = "#9c27b0";
export const THEME_COLOR_DEEP_PURPLE = "#673ab7";
export const THEME_COLOR_BLACK_GRAY = "#212121";
export const THEME_COLOR_GRAY = "#9e9e9e";

//GLOBAL MENU REDUCER
export const OPEN_AND_CLOSE_MENU = "OPEN_AND_CLOSE_MENU";
export const ACTIVE_MENU = "ACTIVE_MENU";
export const SET_ALERT = "SET_ALERT";
export const REMOVE_ALERT = "REMOVE_ALERT";
export const MENU_ACTIVE_STATUS = "default";
export const SET_LOADING = "SET_LOADING";
export const SET_LOADING_DATA = "SET_LOADING_DATA";

//GLOBAL MESSAGE
export const SHOW_MESSAGE = "SHOW_MESSAGE";
export const HIDE_MESSAGE = "HIDE_MESSAGE";
export const MESSAGE_TYPE = "MESSAGE_TYPE";
export const ALERT_TYPE = "ALERT_TYPE";
export const ERROR_FLAG = "error";
export const SUCCESS_FLAG = "success";
export const WARNING_FLAG = "warning";
export const INFO_FLAG = "info";

//GLOBAL ACCOUNT REDUCER
export const ACCOUNT_INFO_WRITE_DATA = "ACCOUNT_INFO_WRITE_DATA";
export const ACCOUNT_INFO_CLEAR_DATA = "ACCOUNT_INFO_CLEAR_DATA";

//GLOBAL HANDLE REDUCER
export const HANDLE_MODE = "HANDLE_MODE";
export const MANAGE_PERMISSION_GROUP = "MANAGE_PERMISSION_GROUP";
export const MANAGE_PERMISSION_ROLE = "MANAGE_PERMISSION_ROLE";
export const MANAGE_PERMISSION_FUNC = "MANAGE_PERMISSION_FUNC";
export const MANAGE_REGISTER = "MANAGE_REGISTER";
export const ACTIVE_REGISTER_MODE = "ACTIVE_REGISTER_MODE";
export const MANAGE_SECTION = "MANAGE_SECTION";
export const MANAGE_ROLE = "MANAGE_ROLE";
export const DASHBOARD_WIDGET_1 = "UsageStatusLogs";
export const DASHBOARD_WIDGET_2 = "";
export const DASHBOARD_WIDGET_3 = "";
export const DASHBOARD_WIDGET_4 = "StatisticRequestTokenLogs";
export const DASHBOARD_WIDGET_5 = "StatisticUsageLogs";
export const DASHBOARD_WIDGET_6 = "UsageOfCountry";

export const NEW = "NEW";
export const EDIT = "EDIT";
export const REMOVE = "REMOVE";

export const REQUEST = "REQUEST";
export const INSERT = "INSERT";
export const UPDATE = "UPDATE";
export const DELETE = "DELETE";

//GLOBAL TYPE REDUCER
export const TYPE_TOKEN_KEY = "TokenKey";
export const TYPE_USER_GROUP = "UserGroup";
export const TYPE_USER = "User";
export const TYPE_FUNCTION_MENU = "Function";
export const TYPE_PERMISSION_GROUP = "TYPE_PERMISSION_GROUP";
export const TYPE_PERMISSION_ROLE = "TYPE_PERMISSION_ROLE";
export const TYPE_PERMISSION_FUNC = "TYPE_PERMISSION_FUNC";
export const TYPE_REGISTER = "Registered";
export const TYPE_SECTION = "Section";
export const TYPE_ROLE = "Role";
export const TYPE_DASHBOARD = "Dashboard";


