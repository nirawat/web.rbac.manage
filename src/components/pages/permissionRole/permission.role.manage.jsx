...
import { TYPE_PERMISSION_ROLE } from "../../../redux/reducers/global.action.type";
...
const ManagePermissionRole = () => {
  ...
  const _permissionRole = useSelector((state) => state.reducerPermissionRole);
  ...
  function asyncData() {
    (async () => {
      let resp = await globalRBACService.PermissionRoleGetItem(
        dispatch,
        TYPE_PERMISSION_ROLE,
        _permissionRole.groupCode
      );
      if (resp.status === 200) {
  ...
  useEffect(
    () => {
      if (
        typeof _permissionRole.groupCode === "undefined" ||
        _permissionRole.groupCode === ""
      ) {
        history.push("/auth/PermissionRole");
      }
      asyncData();
  ...
  const handleClickSaveEvent = async (e) => {
    e.preventDefault();
    let resp = await globalRBACService.PermissionRoleUpdate(
      _permissionRole.groupCode,
      itemRight
    );
    if (resp.status === 200) {
  ...
  const customList = (title, items, heightScale) =>
    isLoadingData ? (
      ...
      <Card style={{ height: heightScale }}>
        ...
        >
          {items.map((value) => {
            const labelId = `transfer-list-all-item-${value.role_code}-label`;
            return (
              <ListItem
                key={value.role_code}
                role="listitem"
  ...
  return (
    <div className={classes.root}>
      ...
      {console.log(itemLeft)}
        <Grid item xs={5} sm={5}>
          {customList(t("permission_role_unselected"), itemLeft, heightScale)}
        </Grid>
        <Grid item xs={2} sm={2}>
          <Grid container direction="column" alignItems="center">
            ...
            <Button
              ...
              disableElevation
              className={classes.button}
              onClick={(e) => history.push("/auth/PermissionRole")}
            >
        ...
        <Grid item xs={5} sm={5}>
          {customList(
            `${t("permission_role_selected")} ${_permissionRole.groupName}`,
            itemRight,
            heightScale
          )}
        </Grid>
